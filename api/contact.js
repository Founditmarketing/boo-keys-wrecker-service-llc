...
module.exports = async (req, res) => {
  ...
  const to   = process.env.CONTACT_TO_EMAIL   || "bookeyswreckerservice316@yahoo.com";
  const from = process.env.CONTACT_FROM_EMAIL || "Boo Key's Wrecker Service <leads@bookeyswrecker.com>";

  const subject = `New lead from bookeyswrecker.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on bookeyswrecker.com`;
  const html = `
<h2>New lead from the Boo Key's Wrecker Service site</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on bookeyswrecker.com</p>`;
